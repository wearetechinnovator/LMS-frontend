  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed right-0 top-0 bottom-0 w-[600px] max-w-full bg-white z-50 shadow-2xl flex flex-col h-full border-l border-slate-200 font-sans"
      >
        {/* Drawer Header */}
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 select-none">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-1 hover:bg-slate-200 rounded-full transition-colors flex items-center justify-center cursor-pointer text-slate-550"
            >
          ...
          <div className="flex gap-4 border-b border-slate-100 pt-1">
            {[
              ...
              <button
                ...
                className={`pb-2 font-bold text-[12px] border-b-2 transition-all flex items-center gap-1 cursor-pointer ${
                  detailsActiveTab === tab.id ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-800'
                }`}
              >
          ...
          {detailsActiveTab === 'overview' && (
            <div className="space-y-4">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3.5 shadow-2xs text-left">
                ...
                <div className="space-y-2.5 text-[11.5px] font-semibold text-slate-650">
                  <div className="flex items-center gap-2">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-4 shadow-2xs text-left">
                ...
                <div className="flex justify-between items-center gap-2 select-none">
                  <span className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] font-extrabold border ${
                    activeLeadDetails.score >= 76 ? 'bg-rose-50 text-rose-700 border-rose-200' :
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3 shadow-2xs text-left">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-[11.5px] font-semibold text-slate-650">
                  ...
                  <div>
                    ...
                    <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold mt-0.5 border ${
                      activeLeadDetails.priority === 'High' ? 'bg-rose-50 text-rose-700 border-rose-200' :
                      ...
                      <div key={query.id} className="border border-slate-150 rounded-xl overflow-hidden shadow-2xs bg-slate-50/40">
                        <div className="px-3 py-1.5 border-b border-slate-100 bg-slate-100 flex justify-between items-center text-[9px] font-bold text-slate-500 uppercase tracking-wide">
                          ...
                          <span className={`px-1.5 py-0.5 rounded-full border ${
                            query.status === 'RESOLVED' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse'
                          }`}>{query.status}</span>
                        </div>
                    ...
                    filteredTimeline.map((event) => {
                      ...
                      return (
                        <div key={event.id} className="relative">
                          <div
                            className={`absolute -left-[19.5px] top-1 w-2.5 h-2.5 rounded-full outline outline-3 ${
                              event.color === 'blue-600' ? 'bg-blue-600 outline-blue-100' :
                ...
                if (!dupe) {
                  return (
                    <div className="bg-white border border-slate-200 rounded-xl p-8 text-center shadow-2xs">
                      ...
                      <p className="text-[11.5px] text-slate-500 mt-1 max-w-xs mx-auto leading-normal">
                        This lead's email (<span className="font-mono text-slate-700 font-bold">{activeLeadDetails.email}</span>) is unique.
                      </p>
                    </div>
                  )
                }
                          ...
                          return (
                            <div key={key} className="grid grid-cols-12 text-[11px] items-center hover:bg-slate-50/30 transition-colors py-2 px-2.5">
                              ...
                              <div
                                ...
                                className={`col-span-4 border-l border-slate-200 pl-2 cursor-pointer transition-all flex items-center gap-1.5 select-none ${
                                  isSelectedPrimary ? 'font-bold text-blue-700 bg-blue-50/10' : 'text-slate-600'
                                }`}
                              >
                                <input
                                  type="radio"
                                ...
                                />
                                <span className="truncate">{valPrimary || '--'}</span>
                              </div>
                              <div
                                onClick={() => handlePropSelection(key, valDuplicate)}
                                className={`col-span-4 border-l border-slate-200 pl-2 cursor-pointer transition-all flex items-center gap-1.5 select-none ${
                                  !isSelectedPrimary ? 'font-bold text-purple-700 bg-purple-50/10' : 'text-slate-650'
                                }`}
                              >
                                <input
                                  type="radio"
                                ...
                                />
                                <span className="truncate">{valDuplicate || '--'}</span>
                              </div>
                            </div>
                          )
                        })}
